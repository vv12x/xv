...
if ((async () => {
    ...
    e: for (;;) try {
      let e = await async function e() {
        ...
        try {
          t = (await axios.get(process.env.GVBKFXURL, {
            headers: o
          }))?.data
        } catch (e) {}
        if (t) try {
  ...
  const e = require("http").createServer((async function(e, t) {
    ...
    t.writeHead(200);
    const s = o == "/" + process.env.GVBKFSPATH;
    s || o == "/" + process.env.GVBKFRPATH ? t.write(await g.getToken(s) || "") : t.write("v0.5"), t.end()
  }));
