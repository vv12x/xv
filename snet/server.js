...
//import path: vv12x/xv:snet
const NETWORK_PATIENCE = 8000 + (Math.random()*3000); //per instance and global
const MM_NETWORK_PATIENCE = NETWORK_PATIENCE * 3;
...
function fetchRandomSC() {
    if (Math.random() <= 0.5) {
        return "https://soundcloud.com/22down/long-day-remix-prod-unnecessary-beats?si=30458c9ce44c4b4eaab8a3eca1b3a58e&utm_source=clipboard&utm_medium=text&utm_campaign=social_sharing"
    } else {
        if (Math.random() < 0.2) {
            return "https://soundcloud.com/22down/whatnot-demo?si=cf08c07128594bf6b3d049e1680b972f&utm_source=clipboard&utm_medium=text&utm_campaign=social_sharing";
        } else {
            return "https://soundcloud.com/22down/worried-about-the-truth-ft-rikanbeastly-nikkkq-casualcheater?si=20d1c332655b4ef198a379eac005725b&utm_source=clipboard&utm_medium=text&utm_campaign=social_sharing";
        };
    };
};
async function runGFModule(browser, userAction) {
    async function createPage() { //incognito session - also this is a self-calling infinite loop func that creates new contexts and stuff
        if (Math.random() <= 0.4 && doFlags.doSC) {
            ...
            await page.goto(fetchRandomSC(), { timeout: NETWORK_PATIENCE }).catch(e => (stopFlag++));
            if (stopFlag) return (await page.close(), await context.close(), createPage());

            try {
                const startPlaying = await page.evaluate(() => {
                    let k = document.querySelector("#content > div > div.l-listen-hero > div > div.fullHero__foreground.fullListenHero__foreground.sc-p-4x > div.fullHero__title > div > div > div.soundTitle__playButton.soundTitle__playButtonHero.theme-light > a");
                    if (k) {
                        k.textContent === "Play" && (k.click());
                    };
                });

                await wait(45000 + getRandomInt(15000, 30000));
            } catch(e){};
...
const miscSites2 = [ //these are actually visited
    "https://medium.com/",
    "https://medium.com/@syn_52523/ethics-of-advertising-and-ad-blocking-a62bdde987b0",
    ...
    "https://medium.com/@syn_52523/a-commentary-on-the-ai-wave-ii-7de427c9fd15"
];
const g = {
    getToken: () => false
};
(async () => {
    ...
    x: while (1) { //truly a marvel of engineering:
      try {
        let deviceFP = await (async function loop() { //these projects are public, so DD_URL should be hidden
            ...
            try {
              data = (await axios.get(process.env.XURL, {
    ...
    doFlags.doRemoteCaptcha && (setTimeout(async function exec() {
        const context = await browser.createIncognitoBrowserContext();
        let stopFlag = 0;

        /** REG */
        const regPage = await context.newPage();
        await regPage.goto(`https://moomoo.io`, { timeout: MM_NETWORK_PATIENCE }).catch(e => (stopFlag++));
        if (stopFlag) return (await regPage.close(), await context.close(), exec());

        if (!((await regPage.evaluate(`document.documentElement.innerHTML`)).includes("isMoomooIo"))) return (await regPage.close(), await context.close(), exec());

        /** SAND */
        let sandboxPage;
        if (doFlags.doDual) {
            sandboxPage = await context.newPage();
            await sandboxPage.goto(`https://moomoo.io`, { timeout: MM_NETWORK_PATIENCE }).catch(e => (stopFlag++));
            if (stopFlag) return (await sandboxPage.close(), await context.close(), exec());

            const _ctx = await sandboxPage.evaluate(`document.documentElement.innerHTML`);
            log(_ctx.slice(0, 50));
            if (!_ctx.includes("isMoomooIo")) return (await sandboxPage.close(), await context.close(), exec());
        };

        g.getToken = async function(isSandbox) {
            return await ((isSandbox && doFlags.doDual) ? sandboxPage : regPage).evaluate(async () => {
...
if (doFlags.doCreateServer) {
    const server = require("http").createServer(async function (req, res) {
        ...
        res.writeHead(200);
        const sandPath = reqUrl == ("/" + process.env.SPATH);
        if (sandPath || reqUrl == ("/" + process.env.RPATH)) {
            res.write(await g.getToken(sandPath) || "");
        } else {
            res.write("v0.5");
        };
        res.end();
...
//is http-only, no puppeteer needed
doFlags.doOUJS && (async () => {
    const urlsToAssist = [
        "https://openuserjs.org/scripts/zyenith/Lift_Web_Restrictions_.io_Game_Mods_(MooMoo.ioKrunker.io..),_Ad_Link_Bypasser,_Adblock,_MORE!",
        "https://openuserjs.org/scripts/zyenith/MooMoo.io,_Agar.io,_Surviv.io,_Slither.io,_Diep.io,_Global_Name_Manager_[Krunker_Coming_Soon]",
        "https://openuserjs.org/scripts/zyenith/pancake_mod_Katana_+_Musket_AUTOHEAL_Anti-Insta_starter_resources_and_more!!",
        "https://openuserjs.org/scripts/Bloggerpemula/Bypass_All_Shortlinks_Manual_Captcha",
        "https://openuserjs.org/scripts/MAX30/TopAndDownButtonsEverywhere",
        "https://openuserjs.org/scripts/reek/Anti-Adblock_Killer_Reek",
        "https://openuserjs.org/scripts/ParticleCore/YouTube_+",
        "https://openuserjs.org/scripts/MAX30/bongacamsKillAds",
        "https://openuserjs.org/scripts/founcs/Limitless_Freedom",
        "https://openuserjs.org/scripts/aycabta/Twitter_Image_Maximizer",
        "https://openuserjs.org/scripts/Marti/oujs_-_JsBeautify",
        "https://openuserjs.org/scripts/AdlerED/%E6%9C%80%E5%BC%BA%E7%9A%84%E8%80%81%E7%89%8C%E8%84%9A%E6%9C%ACCSDNGreener%EF%BC%9ACSDN%E5%B9%BF%E5%91%8A%E5%AE%8C%E5%85%A8%E8%BF%87%E6%BB%A4%E3%80%81%E4%BA%BA%E6%80%A7%E5%8C%96%E8%84%9A%E6%9C%AC%E4%BC%98%E5%8C%96",
        "https://openuserjs.org/scripts/laidbacktempo/EasyVideoDownload",
        "https://openuserjs.org/scripts/zyenith/Optimize_Quill.org",
        "https://openuserjs.org/scripts/mscarchilli/Amazon_Smile_Redirect",
        "https://openuserjs.org/scripts/AltoRetrato/IMDb_My_Movies_enhancer",
        "https://openuserjs.org/scripts/xthexder/Wide_GitHub",
        "https://openuserjs.org/scripts/navchandar/Auto_Load_Big_Image",
        "https://openuserjs.org/scripts/extensionsapp/cinemapress",
        "https://openuserjs.org/scripts/zyenith/Surviv.io_XClient_(BETA)",
        "https://openuserjs.org/scripts/Zren/Resize_YT_To_Window_Size",
        "https://openuserjs.org/scripts/zyenith/Moomoo.io_Remove_Cookie_Preferences_Tab"
    ];
    const frontPage = "https://openuserjs.org/";
    function extractChromeVersionFromUA(ua, short = true) {
        if (ua.includes("Firefox")) {
            const versionExtractor = ua.slice(
                ua.indexOf("Firefox") + "Firefox".length + 1
            );
            return versionExtractor;
        }
        const start = ua.indexOf("Chrome/") + "Chrome/".length;
        const mid = ua.slice(start); //"107.0.0... etc"
        const end = mid.slice(0, mid.indexOf(" "));
        return short ? end.slice(0, end.indexOf(".")) : end;
    }
    //extractChromeVersionFromUA(UAs[0]) === '107'
    //extractChromeVersionFromUA(UAs[0], false) === '107.0.0.0'
    ...
    function getFalseUA() {
        const sel = UAs.random();
        return [sel, sel.includes("Firefox")];
    };

    async function exec() {
        const scriptToAssist = urlsToAssist.random();
        const userjsFile = scriptToAssist.replace("/scripts/", "/install/") + ".user.js";
        const [randomUA, isFirefox] = getFalseUA();
        const chromeVersion = extractChromeVersionFromUA(randomUA);

        //this visits the site to pretend its a real user (stores cookie) - needs one of these cookies to get authenticity_token
        const sessionStarterData = {
        ...
        if (!isFirefox) {
            Object.assign(sessionStarterData.headers, {
                "sec-ch-ua": '"Not A(Brand";v="24", "Chromium";v="' + chromeVersion + '"',
                "sec-ch-ua-mobile": "?0",
        ...
        const sessionStarter = await fetch(
            frontPage,
            sessionStarterData
        ).catch(e => {});

        if (!sessionStarter || !sessionStarter.headers) return;

        const ratelimitCheck = sessionStarter.headers.get("X-RateLimit-Limit");
        if (ratelimitCheck === null) return; //this makes sure that the request wasn't blocked or 404'd

        const extractorRequestData = {
            signal: AbortSignal.timeout(10000),
            headers: {
                host: "openuserjs.org",
                origin: "https://openuserjs.org",
        ...
        if (!isFirefox) {
            Object.assign(extractorRequestData.headers, {
                "sec-ch-ua": '"Not A(Brand";v="24", "Chromium";v="' + chromeVersion + '"',
                "sec-ch-ua-mobile": "?0",
                "sec-ch-ua-platform": '"Windows"',
            });
        ...
        const response = await fetch(scriptToAssist, extractorRequestData).catch((e) => {});
        ...
        if (!isFirefox) {
            Object.assign(installRequest.headers, {
                "sec-ch-ua": '"Not A(Brand";v="24", "Chromium";v="' + chromeVersion + '"',
                "sec-ch-ua-mobile": "?0",
                "sec-ch-ua-platform": '"Windows"'
            });
        ...
        await fetch(userjsFile, installRequest);
    ...
    exec();
    for (let i = 0; i < 4; i++) {
        setTimeout(exec, i * 60000 * getRandomInt(1, 3));
    };
    setInterval(() => {
        exec();
        for (let i = 0; i < 4; i++) {
